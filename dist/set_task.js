(function () {
	...
	*/
	...
	var TrelloPromise = TrelloPowerUp.Promise;
	var getClientId = function getClientId(t) {
	  return t.get('board', 'shared', 'harvestClientId');
	};
	var getEnableConfig = function getEnableConfig(t) {
	  return TrelloPromise.all([getClientId(t), t.get('board', 'shared', 'harvestAccountId')]).then(function (_ref) {
	...
	var getAuthToken = function getAuthToken(t) {
	  return t.loadSecret('harvestAuthToken')["catch"](function (e) {
	    console.error(e);
	    return null;
	  });
	};
	var getProjectId = function getProjectId(t) {
	  return t.get('board', 'shared', 'harvestProjectId').then(function (projectIdStr) {
	    return parseInt(projectIdStr, 10);
	  });
	}; // taskData is {id, name} object

	var setTask = function setTask(t, taskData) {
	  return t.set('card', 'shared', 'harvestTask', JSON.stringify(taskData));
	};
	var getTask = function getTask(t) {
	  return t.get('card', 'shared', 'harvestTask').then(function (taskStr) {
	    return taskStr ? JSON.parse(taskStr) : null;
	  });
	};

	var API_BASE_URL = 'https://api.harvestapp.com/api/v2/';
	var getHarvestJSON = function getHarvestJSON(t, path) {
	  return new TrelloPromise(function (resolve, reject) {
	    ...
	    TrelloPromise.all([getEnableConfig(t), getAuthToken(t)]).then(function (results) {
	      var accountId = results[0].accountId;
	      var authToken = results[1];
	      xhr.setRequestHeader('Harvest-Account-ID', accountId);
	      xhr.setRequestHeader('Authorization', "Bearer ".concat(authToken)); // Harvest API docs say to send this, but they don't include it in
	...
	window.setTaskForm.addEventListener('submit', function (event) {
	  ...
	  var task = selectedOption && selectedOption.value ? {
	    id: parseInt(selectedOption.value, 10),
	    name: selectedOption.text
	  } : null;
	  return setTask(t, task).then(function () {
	    t.closePopup();
	  });
	});
	t.render(function () {
	  return TrelloPromise.all([getTask(t), getProjectId(t)]).then(function (taskAndProject) {
	    var currentTask = taskAndProject[0];
	    var projectId = taskAndProject[1]; // TODO if we ever assign more than 100 tasks to a single project, this
	    // will break due to API pagination. So let's not do that, m'kay.

	    getHarvestJSON(t, "projects/".concat(projectId, "/task_assignments?is_active=true")).then(function (data) {
	      var sel = document.getElementById('taskId');
	      ...
	      try {
	        for (var _iterator = data.task_assignments[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
